...
from hyped.typing import Float32, Int32, Len, Mapping, Sequence, String
...
class ContentPart(Mapping):
    data: String
    """The data content. Either text, url or base64."""

    type: String
    """Describes the type of :code:`data`. Must be a
    :code:`Literal["text", "image_url", "wav", "mp3"]`

    In case of "image_url", :code:`data` must be a valid url path or a base64 encoded image.

    In case of ["wav", "mp3"], :code:`data` must be a base64 encoded audio file."""


class ChatMessage(Mapping):
    role: String
    content: String | Sequence[ContentPart]


class LogProb(Mapping):
    token: String
    logprob: Float32


class LogProbWithTopK(LogProb):
    top_logprobs: Annotated[Sequence[LogProb], Len(lambda c, l, _: c.top_logprobs)]
...
class OpenAIChatCompletionOutput(Mapping):
    ...
    logprobs: Sequence[LogProbWithTopK] | None
    """
...
class OpenAIChatCompletionConfig(BaseDataProcessorConfig):
    ...
    Whether or not to store the output of this chat completion request for use in openai's
...
class OpenAIChatCompletion(BaseDataProcessor[OpenAIChatCompletionConfig]):
    ...
    def __init__(self, config: None | OpenAIChatCompletionConfig = None, **kwargs) -> None:
        ...
        self.semaphore: None | asyncio.Semaphore = None
        self.client: None | AsyncOpenAI = None

    async def api_call(self, messages: list[dict[str, str]]) -> dict[str, Any]:
    ...
    async def process(
        self, ctx: RunContext, messages: Sequence[ChatMessage]
        ...
        Args:
            ctx (RunContext): The runtime context for the data processor, providing execution
                environment details.
            messages (ChatMessage): The messages that will be passed to the openai client.

        Returns:
            OpenAIChatCompletionOutput: Output conforming to the openai scheme.
        ...
        if self.semaphore is None:
            self.semaphore = (
                asyncio.Semaphore(self.config.max_concurrent_calls)
        ...
        # run the api request with retries on rate limits
